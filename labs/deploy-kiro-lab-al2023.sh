...
#   -- or --
#   bash deploy-kiro-lab-al2023.sh
#   bash deploy-kiro-lab-al2023.sh --region us-west-2
#   bash deploy-kiro-lab-al2023.sh cleanup --region us-west-2
#
# Prerequisites:
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        *)
            ...
            echo "Usage: bash deploy-kiro-lab-al2023.sh [deploy|cleanup] [--region REGION]" >&2
            exit 1
            ;;
    esac
done
...
get_al2023_ami() {
    info "Finding latest Amazon Linux 2023 AMI in $REGION..."
    ...
    if [[ -z "$AMI_ID" || "$AMI_ID" == "None" ]]; then
        # Fallback: search by name pattern (Amazon-owned images)
    ...
    if [[ -z "$AMI_ID" || "$AMI_ID" == "None" ]]; then
        error "Could not find Amazon Linux 2023 AMI in region $REGION"
        exit 1
    fi
...
get_userdata() {
    ...
    if [[ -f "$(dirname "$0")/userdata-kiro-lab-al2023.sh" ]]; then
        userdata_file="$(dirname "$0")/userdata-kiro-lab-al2023.sh"
        ...
        # Inline minimal userdata (Amazon Linux 2023 / dnf)
...
print_summary() {
    echo ""
    echo -e "${GREEN}══════════════════════════════════════════════════════════════${NC}"
    echo -e "${GREEN}  Kiro-LAB Instance Deployed Successfully! (Amazon Linux 2023)${NC}"
    echo -e "${GREEN}══════════════════════════════════════════════════════════════${NC}"
    echo ""
    ...
    echo "  5. Open folder: /home/$SSH_USER/workshop"
    echo ""
    echo -e "  ${BLUE}To clean up all resources later:${NC}"
    echo "    bash deploy-kiro-lab-al2023.sh cleanup"
    echo ""
    echo -e "${GREEN}══════════════════════════════════════════════════════════════${NC}"
}
...
main() {
    echo ""
    echo -e "${BLUE}╔══════════════════════════════════════════╗${NC}"
    echo -e "${BLUE}║   Kiro-LAB EC2 Deploy (Amazon Linux 2023) ║${NC}"
    echo -e "${BLUE}╚══════════════════════════════════════════╝${NC}"
    echo ""
